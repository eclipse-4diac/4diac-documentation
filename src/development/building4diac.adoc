...
. To use 4diac IDE directly under this (development) Eclipse instance you need a type library and templates.  
Both are part of the 4diac IDE repository and can be found in the `data` directory.

In order for the development 4diac IDE to locate these resources, they must be copied into the directory where the target platform of the active workspace is stored.
First, navigate to your cloned 4diac IDE repository folder.  
Open:
...
Open the `.bundle_pool` directory and paste the copied `template` and `typelibrary` folders inside it.


After pasting, save/close the file manager.
Finally, return to Eclipse.
