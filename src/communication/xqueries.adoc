...
There are two possibilities to build 4diac FORTE, either manually by setting the cmake options or by the 4diac build environment.
...
=== Build with 4diac build environment
...
Follow the instructions shown on the GitHub repository of the https://github.com/eclipse-4diac/4diac-fbe[4diac build environment].
The configuration folder of the 4diac build environment also contains an example configuration `test-xquery.txt`.
...
Please have a look at the xref:../installation/installation.adoc#ownFORTE[build instructions] if you do not know how to build your own function block, or use the 4diac build environment.
