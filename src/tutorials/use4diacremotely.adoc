...
. Compile 4diac FORTE for your PLC or other Hardware. 
  Check xref:../installation/overview.adoc#FORTEWindowsUnix[here] for more information.
. Go to the System Configuration of the _BlinkTest_ and change the address from localhost to the IP address of your device. 
  ...
  image:Step3/testee_ip.png[changing the device address]
. Stay in System (default) perspective (that’s the 4diac main view).
. Launch Local Forte
...
. Right‑click one of the devices → Run As → [.button4diac]#Deploy# 
image:Step3/deploy_devices.png[Deploy]
...
* 4diac FORTE does not run on the external device +
  ...
  image:Step3/error.png[Major Download Error pop-up]
. You can activate the monitor system and watch the project. Now you can
see that _true_ and __false__ alternate as before.
