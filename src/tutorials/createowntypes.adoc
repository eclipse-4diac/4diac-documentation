...
Create a new Function Block by selecting: *File → New → Type*
...
image:Step4/basicFB_creation_wizard.png[New Type Wizard - Create a Basic Function Block]

. Your newly created type appears in the System Explorer tree on the left of the window.
...
image:Step4/basicFB_editor_overview.png[General View after Creating a Basic Function Block]
...
image:Step4/basicFB_interface.png[Create Basic Type - Interface]
...
The blue boxes (like Init - ①) are states, and the initial state is shown with a double border. 

The light blue bar attached to a state (for example normalOperation ②) is the Action’s algorithm that runs when the Function Block enters that state.

The green label (such as CNF ③) is the output event that the Action triggers after the algorithm finishes.

The arrows between them represent Events that arrive at the Function Block. 

Conditions are assigned to these arrows. `1` means that it will always change state after the algorithm. Events are consumed only one at a time. 
...
image:Step4/basicFB_ecc_initial.png[ECC view]

. Select and delete everything (Click and Drag) but the _START_ state.
. Add a State from the Palette on the right by Drag and Drop. 
  Change its name to _Onn_ (we cannot name it _On_ because "On" is a reserved keyword) by either double-clicking on it, or selecting and then editing the Properties below.
. Add another State and name it _Off_.
. Add an Action to the state _Onn_ via Drag and Drop from the Palette or right click → Add Action.
. Select nothing by clicking in the background of the central editing area.
 then select the Algorithm tab at the bottom of the window. 
  Here you see all the default algorithms of the Function Block. 
  Delete them all by selecting and pressing Delete, since we don't use them in this example.

. Type the new algorithm block:
+
----
ALGORITHM turnOn
  STATE := TRUE;
...
image:Step4/basicFB_algorithm_tab.png[Algorithm tab]

. Double-click on the light blue box of the action of the _Onn_ state and select the _turnOn_ algorithm. 
  Alternatively, you can also select this in the Property view.
. Double-click on the green box of the action of the _Onn_ state and select the _CNF_ output event. 
. Add another algorithm called _turnOff_. 
...
END_ALGORITHM
----
. Double-click on the light blue box of the action of the _Off_ state and select the _turnOff_ algorithm.
. Double-click on the green box of the action of the _Off_ state and select the _CNF_ output event.
+
image:Step4/basicFB_action_select_algorithm.png[Select the correct algorithm]
+
After completing these steps, your ECC should look as follows:
+
image:Step4/basicFB_ecc_complete.png[View after changes]

. Draw a transition via Drag and Drop from the _START_ to the state _Onn_. 
...
. Draw a transition by Drag and Drop from the state _Onn_ to the state _Off_. 
  Select it, and in the transition tab of the Properties view select the condition _REQ_.
. Draw a transition by Drag and Drop from the state _Off_ to the state _Onn_. 
  Select it, and in the transition tab of the Properties view select the condition _REQ_.

image:Step4/basicFB_ecc_with_transitions.png[States with transitions]

At the beginning, the FB is in the START state. 
When a REQ event arrives, it jumps to the _Onn_ state, executes algorithm _turnOn_ and triggers the _CNF_ output event. 
...
image:Step4/simpleFB_creation_wizard.png[New Type Wizard - Create a Simple Function Block]

. Change the interface as link:#changeInterface[before]. 
+ 
image:Step4/simpleFB_change_interface.png[Create Simple Type - Interface]
...
+
image:Step4/simpleFB_algorithm_tab.png[Simple FB Algorithm example]
...
image:Step4/compositeFB_wizard.png[New Type Wizard - Create a Composite Function Block]

. Change the interface as link:#changeInterface[before]. 
+ 
image:Step4/compositeFB_change_interface.png[Create Composite Type - Interface]
. Select the *FB Network tab* at the bottom of the central editing area. 
...
+
image:Step4/compositeFB_network.png[Composite Network example]
...
image:Step4/serviceInterface_wizard.png[New Type Wizard - Create a Service Interface Function Block]

Change the interface as link:#changeInterface[before].

image:Step4/serviceInterface_change_interface.png[Create Service Interface Type - Interface]
