...
This tutorial shows how OPC UA can be used to deploy an IEC 61499 Application. Before the OPC UA deployment can be used it is necessary to xref:../installation/installation.adoc[build a 4diac FORTE] with the xref:../communication/opcua.adoc[OPC UA protocol] enabled. Alternatively, https://github.com/eclipse-4diac/4diac-fbe[4diac FBE] can be used to build a FORTE with OPC UA configuration.
...
Exporting custom FBs works the same way as with the regular deployment profile. A tutorial can be found xref:createowntypes.adoc[here]. A description of building custom modules can be found on the xref:../installation/installation.adoc[build a 4diac FORTE] page, or in the https://github.com/eclipse-4diac/4diac-fbe[4diac FBE description], depending on the used setup.
