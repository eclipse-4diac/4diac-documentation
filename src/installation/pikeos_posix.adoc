...
image:./img/set_src_bin.svg[source code:
...
image:./img/set_crossc.svg[Unix Makefiles cross-compiling options,title="Setting cross-compiling"]

Push _Next_, set "Operating System" to "Generic" and leave all other fields blank.

image:./img/set_os.svg[OS:Generic,title="Setting OS"]
...
image:./img/set_forte_arch.svg[FORTE<em>ARCHITECTURE=PikeOS</em>Posix,title="Setting FORTE_ARCHITECTURE"]
